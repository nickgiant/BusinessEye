...
public class PlasticTabbedPaneUI extends BasicTabbedPaneUI  implements Constants
{
  ...
  /**
   * En este poligono se guarda la forma de la pestaρa. Es muy importante. 
   */
  ...
  protected void installDefaults() 
  {
    ...
    selectColor = new Color(192,192,192); // grey, unselected
  ...
  protected void paintTabBackground( Graphics g, int tabPlacement,int tabIndex,int x, int y, int w, int h, boolean isSelected ) {
    ...
    switch( tabPlacement ) {
      ...
      case RIGHT:
        ...
        yp = new int[]{ y, y+h-3, y+h-3, y,     y};
        gradientShadow = new GradientPaint( x, y, new Color(0,0,255),
                                            x, y+h, new Color(100,100,100));//(153,186,243));
        break;
      case BOTTOM:
      ...
      default:
       ...
       //gradientShadow = new GradientPaint( 0, 0,  new Color(255,255,255),0, y+h/4, new Color(100,100,100));//(153,186,243));
        gradientShadow = new GradientPaint( x, y,CLR_PANEL_START,x, y+h,CLR_PANEL_END); //( x, y,CLR_PANEL_START_ALTER,x, y+h,CLR_PANEL_END_ALTER);//Color.WHITE, x, y+h,new Color(0,128,255));
        break;
    };
    ...
    // Encima, pintamos la pestaρa con el color que sea
    g2D.fill( shape);
    
    // Encima, pintamos la pestaρa con el color que le corresponde por profundidad
    if ( runCount > 1 ) {
      g2D.setColor( hazAlfa( getRunForTab( tabPane.getTabCount(), tabIndex)-1));
      g2D.fill( shape);
    }
    
    // Y despues, le damos un sombreado que hace que parezca curbada (ΏA que duele ver algunas faltas de ortografia?)
   
    g2D.fill( shape);
  }

   /**
    * Este metodo devuelve un tamaρo mas grande de lo necesario, haciendoer hueco para
    * la decoracion.
    */
  protected int calculateTabWidth( int tabPlacement, int tabIndex, FontMetrics metrics) {
   ...
   /**
    * Este metodo devuelve un tamaρo mas grande de lo necesario, haciendo el hueco para
    * la decoracion.
    */
  ...
  protected void paintTabBorder( Graphics g, int tabPlacement, int tabIndex, int x, int y, int w, int h, boolean isSelected)
  {
   ...
   /**
    * Este metodo dibuja una seρal amarilla en la solapa que tiene el foco
    */

      ...
      // invoiceErs[6]=new EntityFilterSettings("πλήθος παρ/κών","","double","fromto","invcount","d",null,"",2,-1,-1,FIELD_NOCOMPLETION);
       ...
       reportBandCustomerServiceSaleDoc[1] = new EntityReportBand("saleline","υπηρεσίες","saleline",null/*entityReportBandFieldsSaleLineA*/,saleFileLineOrderby,"",ENTITYREPORT_QUERY_TYPE_MAIN,boolSettingsReportCustomerfileB,entityPanelSale,null,null);//,"","");                  
        ...
        entReportServiceSaleDoc = new EntityReport("servicesaledoc", REPORT_CAT_1,reportBandCustomerServiceSaleDoc,"SELECT * FROM actionseries, customer, saleheader, saleline, stock WHERE saleheader.customerId = customer.customerId AND saleheader.saleHeaderId = saleline.saleHeaderId AND saleline.stockId = stock.stockId AND actionseries.actionseriesId = saleheader.actionseriesId AND customer.dbCompanyId = saleline.dbCompanyId AND saleheader.dbCompanyId = customer.dbCompanyId AND saleheader.dbCompanyId = actionseries.dbCompanyId AND stock.dbCompanyId = customer.dbCompanyId AND customer.dbCompanyId LIKE "+VariablesGlobal.globalCompanyId+"",""/*ORDER BY saleline.inc"*/,"FORM","μαζική εκτύπωση παρ/κών","",salesDocumentErs,saleDocumentGroupOfComps,invoicesaSelected, null,
                ...
                " WHERE  saleheader.customerId = customer.customerId AND saleheader.saleHeaderId = saleline.saleHeaderId AND actionseries.printFormId = printform.printFormId AND actionseries.actionseriesId = saleheader.actionseriesId AND actionseries.dbCompanyId = printform.dbCompanyId AND saleheader.dbCompanyId = printform.dbCompanyId AND actionseries.dbCompanyId = saleheader.dbCompanyId AND customer.dbCompanyId = saleline.dbCompanyId AND saleheader.dbCompanyId = customer.dbCompanyId AND actionseries.isActive = 1 AND printform.isActive = 1 AND printform.dbCompanyId LIKE "+VariablesGlobal.globalCompanyId+" ","printform.printFormLaser","Θα πρέπει να επιλέξετε φόρμα στο αντίστοιχο παραστατικό.",intSettingsReportCustomerfile,boolSettingsReportDoc,"actionseries.askForPrint");//,globalYearPlusOne) ;

    ...
    public void addMetricsNodes() 
  {
    ...
    public static final int GRAPH_TYPE_LINE=3       
      */ 
        ...
        //entityDockableGraph1[0] =new EntityDockableGraph("πορεία ποσών τοπ 6 εταιριών",GRAPH_TYPE_LINE,"SELECT c.dbCompanyId AS id, c.title, COUNT(i.dateOfSale) AS count, SUM(i.priceTotal) AS sum, AVG(i.priceTotal) AS average, MIN(i.priceTotal) FROM dbcompany c, saleheader i WHERE c.dbCompanyId=i.dbCompanyId GROUP BY i.dbCompanyId ORDER BY sum LIMIT 6","SELECT saleheader.dbCompanyId AS \"id\", dbyear.dbyearId AS \"χρήση\",COUNT(saleheader.dateOfSale) AS \"πληθ παρ\", SUM(saleheader.priceTotal) AS \"τιμή παρ/κων\", AVG(saleheader.priceTotal) AS averagepricetotal,dbcompany.title AS \"εταιρία\" FROM dbyear, saleheader, dbcompany WHERE dbcompany.dbCompanyId=saleheader.dbCompanyId AND dbyear.dbyearId=saleheader.dbyearId AND dbyear.dbCompanyId=saleheader.dbCompanyId GROUP BY saleheader.dbCompanyId, dbyear.dbyearId ORDER BY saleheader.dbCompanyId, dbyear.dbyearId",1,0,1,1);
        entityDockableGraph1[0] =new EntityDockableGraph("πλήθος παραστατικών τοπ 6 εταιριών",GRAPH_TYPE_LINE,"SELECT c.dbCompanyId AS id, c.title, COUNT(i.dateOfSale) AS count, SUM(i.priceTotal) AS sum, AVG(i.priceTotal) AS average, MIN(i.priceTotal) FROM dbcompany c, saleheader i WHERE c.dbCompanyId=i.dbCompanyId GROUP BY i.dbCompanyId ORDER BY sum LIMIT 6","SELECT saleheader.dbCompanyId AS \"id\", date_format(saleheader.dateOfSale, '%m') AS \"μήνας\",COUNT(saleheader.dateOfSale) AS \"πληθ παρ\", SUM(saleheader.priceTotal) AS \"τιμή παρ/κων\", AVG(saleheader.priceTotal) AS averagepricetotal,dbcompany.title AS \"εταιρία\" FROM dbyear, saleheader, dbcompany WHERE dbcompany.dbCompanyId=saleheader.dbCompanyId AND dbyear.dbyearId=saleheader.dbyearId AND dbyear.dbCompanyId=saleheader.dbCompanyId GROUP BY saleheader.dbCompanyId, date_format(saleheader.dateOfSale, '%m') ORDER BY saleheader.dbCompanyId, date_format(saleheader.dateOfSale, '%m')",1,0,1,1);
